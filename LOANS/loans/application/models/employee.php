...
class Employee extends Person {
    ...
    function get_all($limit = 10000, $offset = 0, $search = "", $order = [])
    {
        ...
        $this->db->from('pdv.employees as employees');
        $this->db->join('pdv.people as people', 'employees.person_id=people.person_id');
    ...
    {
        $query = $this->db->get_where('pdv.employees', array('username' => $username, 'password' => md5($password), 'deleted' => 0), 1);
        if ($query->num_rows() == 1)
        {
            $row = $query->row();
